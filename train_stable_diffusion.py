...
def diversity_loss(latents, use_cosine=False):
    ...
    计算多样性损失，可选使用余弦相似度
    """
    batch_size = latents.size(0)  # 16
    latents_flat = latents.view(batch_size, -1)  # (16, 16384)

    if use_cosine:
        # 使用余弦相似度
        latents_norm = F.normalize(latents_flat, p=2, dim=1)  # (16, 16384)
        similarity = torch.mm(latents_norm, latents_norm.t())  # (16, 16)
    else:
        # 使用原始的点积相似度
...
for epoch in range(n_epochs):
    ...
    for batch in train_dataloader:
        latents = batch["latents"].to(device)  # (16, 4, 64, 64)
        text = batch["text"]

        # 添加噪声
        timesteps = torch.randint(0, num_timesteps, (latents.shape[0],), device=device).long()  # 16
        noisy_latents, noise = model.noise_scheduler.add_noise(latents, timesteps)  # (16, 4, 64, 64)  (16, 4, 64, 64)

        # 使用 CLIP 模型编码文本
        text_inputs = tokenizer(text, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt")
        text_embeddings = text_encoder(text_inputs.input_ids.to(device)).last_hidden_state  # (16, 77, 512)

        # 预测噪声
        noise_pred = model(noisy_latents, timesteps, text_embeddings)  # (16, 4, 64, 64)
        mse_loss = F.mse_loss(noise_pred, noise)
        div_loss = diversity_loss(noisy_latents, use_cosine=True)

        # 计算去噪后的潜在表示
        alpha_t = model.noise_scheduler.alphas[timesteps][:, None, None, None]  # (16, 1, 1, 1)
